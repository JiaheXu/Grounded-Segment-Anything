...
class grounded_sam():
    ...
    def run(self):
        ...
        match_idxs = []
        ms = 100
        for idx, msg in enumerate( left_images[0], 0):
            
            match_idx = []

            for cam_idx in range(0, len(self.cam_topics) ):
                for i in range( max(idx-scope,0), min( idx+scope,len(left_images[cam_idx]) -1) ):
                    # print((depth_images[0][i].header.stamp - left_images[0][idx].header.stamp))
                    # print("diff: ",abs(left_images[cam_idx][i].header.stamp - left_images[0][idx].header.stamp))
                    # print(rospy.Duration(0, 1000))
                    if ( abs(left_images[cam_idx][i].header.stamp - left_images[0][idx].header.stamp) < rospy.Duration(0, ms*1000000)) :
                        match_idx.append(i)

            for cam_idx in range(0, len(self.cam_topics) ):
                for i in range( max(idx-scope,0), min( idx+scope,len(depth_images[cam_idx]) -1) ):
                    # print((depth_images[0][i].header.stamp - left_images[0][idx].header.stamp))
                    if ( abs(depth_images[cam_idx][i].header.stamp - left_images[0][idx].header.stamp) < rospy.Duration(0, ms*1000000)) :
                        match_idx.append(i)
            print(len(match_idx))
            if(len(match_idx)!= 2*len(self.cam_topics)):
                continue
            left_image = []
            depth_image = []
            for cam_idx in range(0, len(self.cam_topics) ):
                left = bridge.imgmsg_to_cv2( left_images[cam_idx][match_idx[cam_idx]] ) #bgra
                left = cv2.cvtColor(left, cv2.COLOR_BGRA2RGB)
                left_image.append( left )

                depth = bridge.imgmsg_to_cv2( depth_images[cam_idx][match_idx[cam_idx+3]] )
                depth = np.array(depth)
        ...
        print("match pairs: ", len(match_idxs))
        self.bagIn.close()
        self.bagOut.close()
